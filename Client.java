...
public class Client {
	...
	public static void main(String args[]) throws IOException {
		...
		myPortNum = 20149 + clientNum;
		
		P2PServer server = new P2PServer(myPortNum);
		new Thread(server).start();
		
		servers = new HashMap<Integer, InetAddress>();
		InetAddress firstServerIP = InetAddress.getByName("eng201-18");
	...
	static void exit(InetAddress destIP, int destPort)throws SocketException, IOException{
		...
		for (int i = 1; i<=4; i++){
			query = new DatagramPacket(exit, exit.length, servers.get(i), destPort-1+i);
			ds.send(query);
		}
	...
	static void getImageFromUser(InetAddress userIP, int userPort, String filename) throws IOException {
		String path = "H:\\Documents\\706Proj\\imgdest\\";
		byte[] imageBytes;
		int imageSize;
		BufferedImage image;
	...
	static void informAndUpdate() throws IOException {
		...
		do{
			...
			String path = "H:\\Documents\\706Proj\\imgsrc\\";
